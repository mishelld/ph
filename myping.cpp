...
int main ()
{
   ...
   dest_in.sin_addr.s_addr = inet_addr(DESTINATION_IP);
    ...
    //*change from IPPROTO_RAW to IPPROTO_ICMP*
    int sock = -1;
   ...
   /* const int flagOne = 1;
    ...
    int by;
    start = clock();
    by = sendto (sock, packet, ICMP_HDRLEN + datalen, 0, (struct sockaddr *) &dest_in, sizeof (dest_in)); 
    //  if (by = sendto (sock, packet, IP4_HDRLEN + ICMP_HDRLEN + datalen, 0, (struct sockaddr *) &dest_in, sizeof (dest_in)) == -1)  
    if(by==-1)
    {
    ...
    printf("1 packet was sent\n");
    printf("bytes sent:%d\ndata:%sdata length:%d\n",by,packet + ICMP_HDRLEN,datalen);
///////////////////////////////////////////////////////////////////////////////////////////////////////////

   ///////////////////////////////////////// ICMP-ECHO-REPLY ///////////////////////////////////////////
   int bytes;
   socklen_t  len=sizeof(dest_in);
     //reset the buffer so thet we will know if we really recived the msg
		bzero(packet, sizeof(packet));
		bytes = recvfrom(sock, packet, sizeof(packet), 0, (struct sockaddr*)&dest_in, &len);
